...
**האופן היחיד שצריך בפועל:**

```powershell
npm run dev:gui
```

ובתוך ה-GUI עצמו לוחצים על **"הפעל שירות"** כדי להריץ את שרת ה-MCP. שרת ה-MCP רץ **באותו תהליך של ה-GUI עצמו** (`startMcpServer()` מיובא ישירות מ-`@tany-desktop/mcp-server`, לא תהליך-בן נפרד) - כך שאין יותר בעיית ABI כפול בין `better-sqlite3` ל-Electron מול Node הרגיל: יש רק build אחד (Electron), ומשתמשים בו לכל דבר. **סגירת החלון לא עוצרת את השירות** - היא רק ממזערת אותו לשורת המשימות (tray); לחיצה על סמל ה-tray (או פתיחת האפליקציה שוב) מחזירה את החלון. יציאה אמיתית רק דרך "יציאה" בתפריט ה-tray.

**להרצת שרת ה-MCP כתהליך עצמאי בלי GUI בכלל** (שימוש מתקדם - למשל Scheduled Task, ראו למטה):
...
**חשוב:** מצב זה **כן** דורש build נפרד (Node ABI, לא Electron) של `better-sqlite3`, ולכן **לא** ניתן להריץ אותו בו-זמנית עם ה-GUI מאותה התקנה בלי לעבור בין ה-builds (`npm rebuild better-sqlite3` ↔ `npx @electron/rebuild -f -w better-sqlite3`) - אבל זה כבר לא נדרש לשימוש רגיל, כי ה-GUI לבדו כבר עושה הכל.
...
**שמירה על ה-session פעיל (keep-alive):** לאתרים שמפוגים session מהר (בנקים בפרט - בד"כ תזמון שרת-צד לפי זמן מאז הבקשה האחרונה, לא לפי חוסר פעילות מקומי של עכבר/מקלדת - לכן שום דבר מקומי לא יכול "לאפס" את זה חוץ מבקשה אמיתית לאתר), שרת ה-MCP מבקר אוטומטית כל כמה דקות (`TANY_DESKTOP_KEEPALIVE_INTERVAL_MS`, ברירת מחדל 5 דקות) בכל שגרת web ששמורה לה session, עם ה-session השמור, כדי לרענן אותו לפני שהוא פג - ראו `packages/engine-web/src/keepAlive.ts`. רץ אוטומטית לכל שגרה כזו מרגע שהשירות פעיל; אין עדיין אפשרות להשבית פר-שגרה בודדת.

### התקנה כשירות רקע קבוע (Auto-start) - שימוש מתקדם

**לרוב המשתמשים זה לא נדרש:** ה-GUI עצמו (ראו למעלה) כבר "נשאר ברקע" דרך ה-tray אחרי סגירת החלון, כל עוד לוחצים "הפעל שירות" פעם אחת - זה מספיק לתרחיש הנתמך (מחשב ייעודי שנשאר דלוק ומחובר, סעיף 9 באפיון). ה-Scheduled Task למטה שימושי בעיקר אם רוצים ששרת ה-MCP יעלה **בלי GUI בכלל וגם בלי שאף אחד יתחבר** למחשב (למשל שרת ייעודי).
