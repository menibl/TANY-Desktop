...
גוגל (וספקי זהות דומים) חוסמים ביודעין כל ניסיון כניסה מדפדפן שמזוהה כאוטומטי (Playwright/WebDriver) - זו מדיניות אבטחה מכוונת מצידם, לא באג בתוכנה, ואי אפשר "לעקוף" אותה בתוך ההקלטה/הרצה האוטומטית עצמה.

**הפתרון:** לפני "התחל הקלטה", לוחצים על **"🔐 התחברות חד-פעמית"**. נפתח דפדפן רגיל (עדיין דרך Playwright, אבל בלי הקלטה בכלל) - מתחברים בו ידנית פעם אחת, וסוגרים את החלון. ה-session (עוגיות) נשמר מוצפן על המחשב (כמו Credential), וההקלטה/ההרצות הבאות מתחילות כבר מחוברות - כך שהן לא נוגעות במסך ה-Sign in של גוגל בכלל. אם ההרצה מתחילה להיכשל כי ה-session פג, יש כפתור "רענון התחברות" גם במסך הדיבאג של השגרה הקיימת.
