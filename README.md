...
גוגל (וספקי זהות דומים) חוסמים ביודעין כל ניסיון כניסה מדפדפן שמזוהה כאוטומטי (Playwright/WebDriver) - זו מדיניות אבטחה מכוונת מצידם, לא באג בתוכנה. **חשוב להבין:** זו לא רק "לא להקליט" - כל דפדפן שמנוהל ע"י Playwright מזוהה ככזה, גם כשמישהו פשוט לוחץ בו ידנית. לכן גם שלב ה"התחברות החד-פעמית" חייב להשתמש בדפדפן פחות חשוד.

**הפתרון (best-effort, לא מובטח):** לפני "התחל הקלטה", לוחצים על **"🔐 התחברות חד-פעמית"**. נפתח **Google Chrome האמיתי המותקן במחשב** (לא ה-Chromium המובנה של Playwright - `--channel chrome`, פחות חשוד בעיני גוגל) - מתחברים בו ידנית פעם אחת, וסוגרים את החלון. **דורש Chrome אמיתי מותקן** (`google.com/chrome`) - אם אין, תופיע הודעת שגיאה מתאימה. ה-session (עוגיות) נשמר מוצפן על המחשב (כמו Credential), וההקלטה/ההרצות הבאות מתחילות כבר מחוברות - כך שהן לא נוגעות במסך ה-Sign in של גוגל בכלל. אם ההרצה מתחילה להיכשל כי ה-session פג, יש כפתור "רענון התחברות" גם במסך הדיבאג של השגרה הקיימת.

אם גם עם Chrome אמיתי גוגל ממשיכה לחסום - זה מגבלה ידועה של הגישה הזו (הזיהוי של גוגל מתוחכם ומשתנה); הצעד הבא שידרוש עוד עבודה הוא ייבוא עוגיות מה-Chrome היומיומי שלכם (שם אתם כבר מחוברים באופן טבעי, ללא כל אוטומציה) במקום לנסות להתחבר מחדש דרך דפדפן שמופעל על ידי התוכנה.
